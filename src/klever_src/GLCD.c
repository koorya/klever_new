...
void SPI_Init(void)
{
	...
	// Подключаем перефирийную систему SPI1 к шине тактирования
	RCC->APB2ENR |= RCC_APB2ENR_SPI1EN;
	
	// Используется SPI2. 
	// Первоначально отключаем работу SPI интерфейса, обнуляем регистр данных (убираем "мусор")
	SPI1->CR1 = 0;
	SPI1->CR2 = 0;
	SPI1->DR = 0;
	
	// Начинаем конфигурировать: 
	// Выборка по заднему нарастающему фронту
	// CPOL = 1 переход из 1 в 0 (в простое на ноге высокий уровень)
	// СНРА = 1 по заднему фронту
	// MSTR = 1 работает в режиме мастера
	// BR[2:0] = 011 устанавливаем предделитель на 16
	// BR[2:0] = 100 устанавливаем предделитель на 32
	// BR[2:0] = 101 устанавливаем предделитель на 64
	// BR[2:0] = 110 устанавливаем предделитель на 128
	// BR[2:0] = 111 устанавливаем предделитель на 256 = SPI_CR1_BR_2 | SPI_CR1_BR_1 | SPI_CR1_BR_0 |
	SPI1->CR1 = SPI_CR1_SSM | SPI_CR1_SSI | SPI_CR1_MSTR | SPI_CR1_BR | SPI_CR1_CPHA | SPI_CR1_CPOL; // SPI_CR1_DFF | 
	
	// включили интерфейс SPI
...
char spi_Send(uint8_t raw)
{
	...
	{
			 // ждем пока не скинется этот бит (не окончится передача данных)
	}
	if (!err_cnt)
		return 1;
	return 0;
